...
% Last Modified by GUIDE v2.5 31-May-2011 23:15:36
...
EC.vol.nn=-0.01;
EC.vol.nx=-10;
EC.vol.null=[0.75,0.75,0.75];
EC.vol.CM=ones(100,3)*0.75;
EC.vol.color_map=1;
...
% Make Display correct in linux - YAN Chao-Gan 120104 Added.
if ~ispc
    ZoomFactor=0.85;
...
if EC.edg.color_abs==1
    ...
    if max(surf.cylinder3(:,3))~=min(surf.cylinder3(:,3))
        ...
        EC.Net.b=1.1-EC.Net.k*max(surf.cylinder3(:,3));
    else
        EC.Net.k=0;
        EC.Net.b=0.7;
    end
...
if max(surf.cylinder(:,3))~=min(surf.cylinder(:,3))
    ...
    EC.Net.b=1.1-EC.Net.k*max(surf.cylinder(:,3));
else
    EC.Net.k=0;
    EC.Net.b=0.7;
end
...
function Viewer=GenerateView4(h1,w1)
...
for i=1:4
    ...
    lighting phong; material dull; shading interp;axis off
    set(Brain,'FaceColor',EC.msh.color);
    set(Brain,'FaceAlpha',EC.msh.alpha);
    if alpha~=1
        lighting phong;axis tight; axis vis3d off;
        cam=camlight;
    end
    switch i
        case 1
...
for i=1:6
    ...
    whitebg(gcf,EC.bak.color);
    set(gcf,'Color',EC.bak.color,'InvertHardcopy','off');
    material dull; shading interp;axis off
    set(Brain,'FaceColor',EC.msh.color);
    set(Brain,'FaceAlpha',EC.msh.alpha);
    if alpha~=1
        axis tight; lighting phong;axis vis3d off;
        cam=camlight;
    end
end


function a=MapMesh6(Viewer,surf,low,high)
global EC
global cam
a=zeros(1,6);
for i=1:6
    ...
    whitebg(gcf,EC.bak.color);
    set(gcf,'Color',EC.bak.color,'InvertHardcopy','off');
    material dull; shading flat;axis off
    colormap(EC.vol.CM);
    caxis([low,high]);
    axis tight; lighting phong;axis vis3d off;
    cam=camlight;
...
function a=MapMesh1(surf,low,high)
global EC
global cam
a=axes;
Brain=trisurf(surf.tri,surf.coord(1,:),surf.coord(2,:),surf.coord(3,:),surf.T,'EdgeColor','none');
...
function a=MapMesh8(Viewer,Surfmatrix,low,high)
global EC
global cam
a=zeros(1,8);
for i=1:8
    ...
    whitebg(gcf,EC.bak.color);
    set(gcf,'Color',EC.bak.color,'InvertHardcopy','off');
    lighting phong; material dull; shading flat;axis off
    lighting phong;axis tight; axis vis3d off;
    cam=camlight;
    switch i
        case 1
...
function a=MapMesh4(Viewer,Surfmatrix,low,high)
% YAN Chao-Gan 111023 Added. For Medium View (4 views)
global EC
global cam
a=zeros(1,4);
...
if length(a)>1
    for i=1:6
        ...
        if n>1
            axis tight; axis vis3d off;material dull;lighting phong;
            cam=camlight;
        end
        hold off
    end
...
global EC
switch i
    case {1,4} %%% Edited by Mingrui Xia, 111028. Adjust label postion.
    ...
    case 2
        if surf.sphere(j,1)<=0
            text(surf.sphere(j,1),surf.sphere(j,2)+surf.sphere(j,7)+2,surf.sphere(j,3),surf.label{j}{1}{1},EC.lbl_font,'HorizontalAlignment','right')
        else
            text(surf.sphere(j,1),surf.sphere(j,2)+surf.sphere(j,7)+2,surf.sphere(j,3),surf.label{j}{1}{1},EC.lbl_font,'HorizontalAlignment','left')
        end
    case 3
        if surf.sphere(j,1)<=0
            text(surf.sphere(j,1),surf.sphere(j,2),surf.sphere(j,3)+surf.sphere(j,7)+2,surf.label{j}{1}{1},EC.lbl_font,'HorizontalAlignment','left')
        else
            text(surf.sphere(j,1),surf.sphere(j,2),surf.sphere(j,3)+surf.sphere(j,7)+2,surf.label{j}{1}{1},EC.lbl_font,'HorizontalAlignment','right')
        end
    case 5
        if surf.sphere(j,1)<=0
            text(surf.sphere(j,1),surf.sphere(j,2)-surf.sphere(j,7)-2,surf.sphere(j,3),surf.label{j}{1}{1},EC.lbl_font,'HorizontalAlignment','right')
...
global EC
switch i
    case {1,3,4,6} %%% Edited by Mingrui Xia, 111028. Combine several same situation, adjust label postion.
        text(surf.sphere(j,1),surf.sphere(j,2),surf.sphere(j,3)+surf.sphere(j,7)+2,surf.label{j}{1}{1},EC.lbl_font,'HorizontalAlignment','center');
        %     case {4,6}
        %         text(surf.sphere(j,1),surf.sphere(j,2),surf.sphere(j,3)+surf.sphere(j,7)*2,surf.label{j}{1}{1},EC.lbl_font,'HorizontalAlignment','center');
        %     case 3
...
switch EC.edg.color
    ...
    case 2
        switch EC.edg.color_abs
            case 0
                ci=int32(45*surf.cylinder(i,3)-3.5);
                if ci<1
                    ci=1;
                elseif ci>64
                    ci=64;
...
if FLAG.IsCalledByREST==1
%     ColorNum=size(EC.edg.CM,1);
%     ci=1+fix((ColorNum-1)*(surf.cylinder2(i,3)-min(surf.cylinder2(:,3)))/(max(surf.cylinder2(:,3))-min(surf.cylinder2(:,3))));
    if surf.cylinder2(i,3)<0
        ci=1;
    else
        ci=2;
    end
            ...
            switch FLAG.Loadfile
                ...
                case 9
                    ...
                    if cut<t
                        Viewer=GenerateView8(h1,w1);
                        Surfmatrix=GenertateSurfM8(surf,tl,tr,vl,vr,cuv);
                        a=MapMesh8(Viewer,Surfmatrix,low,high);
                    else
                        Viewer=GenerateView6(w1);
                        a=MapMesh6(Viewer,surf,low,high);
                    end
            end
        ...
        case 3  %%% YAN Chao-Gan 111023 Added BEGIN. For medium view (4 views). Only finished for volume view mode
            switch FLAG.Loadfile
                case 9
                    ...
                    Viewer=GenerateView4(h1,w1);
                    Surfmatrix=GenertateSurfM4(surf,tl,tr,vl,vr,cuv);
                    a=MapMesh4(Viewer,Surfmatrix,low,high);%%% YAN Chao-Gan 111023 Added END. %%%
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
msgbox({'BrainNet Viewer 1.1 Released 20111028';'By Mingrui Xia';'mingruixia@gmail.com'},'About...','help');
